		...
		SPEED_PENALTY_PERCENT = GameRules.register("speedPenaltyPercent", GameRules.Category.PLAYER, GameRules.IntegerValue.create(100, (server, value) -> {
			...
			for (ServerPlayerEntity player : server.getPlayerList().getPlayers()) {
				...
				if (playerdata != null) {
					...
					for(AttributeModifier attributeModifier : playerdata.getOriginalEntity().getHeldItemMainhand().getAttributeModifiers(EquipmentSlotType.MAINHAND).get(Attributes.ATTACK_SPEED)) {
						mainWeaponSpeed += (float)attributeModifier.getAmount();
					}
					
					mainhandAttackSpeed.applyNonPersistentModifier(new AttributeModifier(ServerPlayerData.WEIGHT_PENALTY_MODIFIIER, "weight penalty modifier",
					...
					for(AttributeModifier attributeModifier : playerdata.getOriginalEntity().getHeldItemOffhand().getAttributeModifiers(EquipmentSlotType.MAINHAND).get(Attributes.ATTACK_SPEED)) {
						offWeaponSpeed += (float)attributeModifier.getAmount();
					}
					
					offhandAttackSpeed.applyNonPersistentModifier(new AttributeModifier(ServerPlayerData.WEIGHT_PENALTY_MODIFIIER, "weight penalty modifier",
